...
-   **Storage Quota & AI Usage Tracking System**: Comprehensive usage tracking for fair billing. Uses `media_assets` table for file tracking (profileId, iceId, fileKey, fileSizeBytes, category, status) and `ai_usage_events` table for AI cost tracking (profileId, iceId, usageType, creditsUsed, model, metadata). Creator profiles track `usedStorageBytes` and `storageLimitBytes` (bigint for >5GB support). Storage limits by tier: Starter 5GB, Creator 25GB, Studio 100GB. AI billing is pay-as-you-go with costs logged per generation (~$0.04/image, ~$0.75/5sec video, ~$0.015/1K chars TTS). API endpoints: `GET /api/me/usage` (storage + AI summary), `GET /api/ice/:id/usage` (per-ICE breakdown), `POST /api/me/storage/check` (pre-upload quota validation). Doctrine documented in `docs/STORAGE_AI_DOCTRINE.md`.
-   **Producer Brief Mode**: Enables professional producers to upload structured specification documents (.docx, .txt, .md) that auto-create ICEs with exact specifications. Key features: (1) Exact card counts from brief (no AI approximation), (2) Auto-created AI characters with personality, system prompts, and stage-specific context additions, (3) Interactivity checkpoints at stage boundaries, (4) Visual direction per card. Schema in `shared/producerBrief.ts`, parser in `server/services/briefParser.ts`, API at `POST /api/ice/preview/brief`. Brief format uses: `Stage N: Name` headers, card tables with Card | Content | Visual columns, `AI Character` sections, `System Prompt` definitions, and `AI Checkpoint` markers. Accessible via `/create` page under "For Producers" section.
-   **Enterprise Custom Branding Page**: Sales and education page at `/enterprise/custom-branding` for enterprise custom branding services. NOT in main navigation - discoverable via contextual CTAs. Features: Hero section, feature blocks (Custom Title Packs, Branded Captions, Custom Music/Sonic Identity, Custom SFX Pack, Brand Asset Library, Done-for-you Media Production), 3-step How It Works, Governance section, and contact form. Contact form submits to `POST /api/enterprise/branding-enquiry`. Pricing positioned as "tailored to requirements". Upsell component (`EnterpriseBrandingUpsell`) placed in: Pricing page Enterprise section, Caption settings, Title pack selector, Audio/music settings, Logo branding panel.
